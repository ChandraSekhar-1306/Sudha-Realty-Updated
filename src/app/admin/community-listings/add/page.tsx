...
export default function AddCommunityListingPage() {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
        ...
        images: [],
        listingType: 'rent',
        propertyType: 'Apartment/Gated Community',
        bhk: '2 BHK',
        bathrooms: 1,
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const listingsCol = collection(firestore, 'community_listings');

    addDoc(listingsCol, values)
        .then(() => {
            toast({
                title: 'Listing Added!',
                description: `The listing "${values.title}" has been successfully added.`,
            });
    ...
    <Card>
      <CardHeader>
        <div className='flex items-center gap-4'>
            ...
            <div>
                <CardTitle>Add New Community Listing</CardTitle>
                <CardDescription>Fill out the form to add a new community listing.</CardDescription>
            </div>
        </div>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <FormField control={form.control} name="preferredTenants" render={({ field }) => (<FormItem><FormLabel>Preferred Tenants</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Select preferred tenants" /></SelectTrigger></FormControl><SelectContent><SelectItem value="Family">Family</SelectItem><SelectItem value="Company">Company</SelectItem><SelectItem value="Male Bachelors">Male Bachelors</SelectItem><SelectItem value="Female Bachelors">Female Bachelors</SelectItem><SelectItem value="Pure Vegetarian Family">Pure Vegetarian Family</SelectItem><SelectItem value="Any">Any</SelectItem></SelectContent></Select><FormMessage /></FormItem>)} />
                 <FormField control={form.control} name="waterSupply" render={({ field }) => (<FormItem><FormLabel>Water Supply</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Select water supply" /></SelectTrigger></FormControl><SelectContent><SelectItem value="Corporation">Corporation</SelectItem><SelectItem value="Borewell">Borewell</SelectItem><SelectItem value="Both">Both</SelectItem></SelectContent></Select><FormMessage /></FormItem>)} />
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <FormField control={form.control} name="gatedSecurity" render={({ field }) => (<FormItem><FormLabel>Gated Security</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger></FormControl><SelectContent><SelectItem value="Yes">Yes</SelectItem><SelectItem value="No">No</SelectItem></SelectContent></Select><FormMessage /></FormItem>)} />
                <FormField control={form.control} name="petAllowed" render={({ field }) => (<FormItem><FormLabel>Pet Allowed</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger></FormControl><SelectContent><SelectItem value="Yes">Yes</SelectItem><SelectItem value="No">No</SelectItem></SelectContent></Select><FormMessage /></FormItem>)} />
            ...
            <Button type="submit" disabled={form.formState.isSubmitting}>
              {form.formState.isSubmitting ? 'Adding...' : 'Add Community Listing'}
            </Button>
          </form>
        </Form>
      </CardContent>
